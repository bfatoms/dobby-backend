...
class InvoiceCreateRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        request()->merge(['exchange_rate' => request('exchange_rate', 1)]);
        ...
        return [
            ...
            'order_lines.*.tax_rate' => [
                function ($attribute, $value, $fail) {
                    if (request('tax_setting') === 'no tax') {
                        if ($value != 0) {
                            return $fail('TAX_RATE_MUST_BE_0_IF_TAX_SETTING_IS_NO_TAX');
                        }
                    }
                },
            ...
            'order_lines.*.tax_rate_id' => [
                ...
                function ($attribute, $value, $fail) {
                    if (request('tax_setting') === 'no tax') {
                        if ($value != 1) {
                            return $fail('TAX_RATE_ID_MUST_BE_TAX_EXEMPT');
                        }
                    }
                },
            ],
            'order_lines.*.chart_of_account_id' => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    if ($account['type'] === 'inventory' && request('is_tracked') == true) {
                        return $fail('INVENTORY_TYPE_ACCOUNT_IS_NOT_ALLOWED_ON_INVOICE');
                    }
                },
            ],
