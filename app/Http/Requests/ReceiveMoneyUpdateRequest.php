...
class ReceiveMoneyUpdateRequest extends FormRequest
{
    /**
    ...
    {
        $this->data = ReceiveMoney::find(request('order'));
        ...
        if ( in_array(request('order_type'), ['RMP', 'RMO']) ) {
            ...
            $validations['order_lines.*.chart_of_account_id'] = [
                ...
                function ($attribute, $value, $fail) {
                    $payable = ChartOfAccount::where('system_name', 'accounts-payable')->first();
                    if ($value != $payable['id']) {
                        $fail('CHART_OF_ACCOUNT_MUST_BE_ACCOUNTS_PAYABLE');
                    }
                }
            ];
        }
